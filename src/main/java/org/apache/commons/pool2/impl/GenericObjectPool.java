 ...
 * abandonment is determined by how long an object has been checked out from the
 * pool.
 * </p>
 * <p>
 ...
 * @since 2.0
 */
public class GenericObjectPool<T> extends BaseGenericObjectPool<T> implements
	 ...
	 * 
	 * @param factory
	 *            The object factory to be used to create object instances used
	 *            by this pool
	 */
	 ...
	 *            by this pool
	 * @param config
	 *            The base pool configuration to use for this pool instance. The
	 *            configuration is used by value. Subsequent changes to the
	 *            configuration object will not be reflected in the pool.
	...
	/**
	 * Sets the target for the minimum number of idle objects to maintain in the
	 * pool. This setting only has an effect if it is positive and
	 ...
	 * 
	 * @return true if this pool is configured to detect and remove abandoned
	 *         objects
	 */
	@Override
	...
	public void setConfig(GenericObjectPoolConfig conf) {
		...
		setTimeBetweenEvictionRunsMillis(conf
				.getTimeBetweenEvictionRunsMillis());
		setSoftMinEvictableIdleTimeMillis(conf
	...
	public void setAbandonedConfig(AbandonedConfig abandonedConfig)
		...
		if (abandonedConfig == null) {
			...
			this.abandonedConfig.setLogAbandoned(abandonedConfig
					.getLogAbandoned());
			this.abandonedConfig.setLogWriter(abandonedConfig.getLogWriter());
			this.abandonedConfig.setRemoveAbandonedOnBorrow(abandonedConfig
					.getRemoveAbandonedOnBorrow());
			this.abandonedConfig
					.setRemoveAbandonedOnMaintenance(abandonedConfig
							.getRemoveAbandonedOnMaintenance());
			this.abandonedConfig.setRemoveAbandonedTimeout(abandonedConfig
					.getRemoveAbandonedTimeout());
			this.abandonedConfig.setUseUsageTracking(abandonedConfig
					.getUseUsageTracking());
		}
	}
	 ...
	 * <p>
	 * If there is one or more idle instance available in the pool, then an idle
	 * instance will be selected based on the value of {@link #getLifo()},
	 * activated and returned. If activation fails, or
	 * {@link #getTestOnBorrow() testOnBorrow} is set to <code>true</code> and
	 * validation fails, the instance is destroyed and the next available
	 * instance is examined. This continues until either a valid instance is
	 * returned or there are no more idle instances available.
	 * <p>
	 ...
	 * <code>NoSuchElementException</code> (if {@link #getBlockWhenExhausted()}
	 * is false). The length of time that this method will block when
	 * {@link #getBlockWhenExhausted()} is true is determined by the value
	 * passed in to the <code>borrowMaxWaitMillis</code> parameter.
	 * <p>
	 ...
	 *            The time to wait in milliseconds for an object to become
	 *            available
	 * 
	...
	public T borrowObject(long borrowMaxWaitMillis) throws Exception {
		...
		while (p == null) {
			...
			if (p != null) {
				...
				if (p != null
						&& (getTestOnBorrow() || create && getTestOnCreate())) {
	 ...
	 * notified via a {@link SwallowedExceptionListener}.</li>
	 * </ul>
	 */
	@Override
	public void clear() {
	 ...
	 * <p>
	 * If there are {@link #getMaxTotal()} objects already in circulation or in
	 * process of being created, this method returns null.
	 * 
	 ...
	 * <p>
	 * Creates and adds idle instances until either {@link #getNumIdle()}
	 * reaches {@code idleCount} or the total number of objects (idle, checked
	 * out, or being created) reaches {@link #getMaxTotal()}. If {@code always}
	 * is false, no instances are created unless there are threads waiting to
	 * check out instances from the pool.
	 * 
	 * @param idleCount
	 *            the number of idle instances desired
	 * @param always
	 *            true means create instances even if the pool has no threads
	 *            waiting
	 * @throws Exception
	...
	private void ensureIdle(int idleCount, boolean always) throws Exception {
		if (idleCount < 1 || isClosed()
				|| (!always && !idleObjects.hasTakeWaiters())) {
			return;
		}
	...
	/**
	 * Recover abandoned objects which have been checked out but not used since
	...
	public String getFactoryType() {
		...
		if (factoryType == null) {
			...
			result.append('<');
			Class<?> pooledObjectType = PoolImplUtils.getFactoryType(factory
					.getClass());
	...
	public Set<DefaultPooledObjectInfo> listAllObjects() {
		Set<DefaultPooledObjectInfo> result = new HashSet<DefaultPooledObjectInfo>(
				allObjects.size());
	 ...
	 * excludes objects that have been destroyed. The size of {@link
	 * #allObjects} will always be less than or equal to {@link #_maxActive}.
	 * Map keys are pooled objects, values are the PooledObject wrappers used
	 * internally by the pool.
	 */
	protected final Map<T, PooledObject<T>> allObjects = new ConcurrentHashMap<T, PooledObject<T>>();
	/*
	 * The combined count of the currently created objects and those in the
	 * process of being created. Under load, it may exceed {@link #_maxActive}
	 * if multiple threads try and create a new object at the same time but
	 * {@link #create()} will ensure that there are never more than {@link
	 * #_maxActive} objects created at any one time.
	 */
	private final AtomicLong createCount = new AtomicLong(0);
	protected final LinkedBlockingDeque<PooledObject<T>> idleObjects = new LinkedBlockingDeque<PooledObject<T>>();
